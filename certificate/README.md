## Electrical Engineering Courses

> - Aything related to Electrical, Electronics, or Computer Engineering.

<br />
<br />

## Computer Science Courses

> - Anything related to Computer Science or Software Engineering.

<br />

`Temporal` | `Temporal 101 with Go` | [`09/05/2025`](./files/Temporal-101-with-Go.png) <br />
`Snyk` | `OWASP` | [`07/11/2025`](https://github.com/kentlouisetonino/kentlouisetonino/blob/develop/certificate/files/Snyk-OWASP.pdf) <br />
`Linux Foundation` | `A Beginner's Guide to Linux Kernel (LFD103)` | [`10/29/2024`](https://ti-user-certificates.s3.amazonaws.com/e0df7fbf-a057-42af-8a1f-590912be5460/2efdabb6-d9c6-42c9-8498-9af47537523b-kent-tonino-8f3318f9-dd80-4c4d-bcae-98d83cc992a5-certificate.pdf) <br />
`Snyk` | `Security for Developers` | [`10/08/2024`](https://github.com/kentlouisetonino/kentlouisetonino/blob/develop/certificate/files/Snyk-Security-for-Developers.pdf) <br />
`Sololearn` | `C Intermediate` | [`08/28/2023`](https://www.sololearn.com/en/certificates/CC-V81MRQU9) <br />
`Sololearn` | `Introduction to C` | [`07/06/2023`](https://www.sololearn.com/certificates/CC-6AGYI9YG) <br />
`FreeCodeCamp` | `Backend Development and APIs` | [`03/26/2023`](https://www.freecodecamp.org/certification/kentlouisetonino/back-end-development-and-apis) <br />
`Sololearn` | `Python Core` | [`10/10/20211`](https://www.sololearn.com/certificates/CT-UOJ7MU3L) <br />
`Sololearn` | `SQL` | [`09/17/2021`](https://www.sololearn.com/en/certificates/CT-OYPTHJVE) <br />
